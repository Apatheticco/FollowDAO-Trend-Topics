...
[C 给标的/句子] → 第 -1.5 步：定向检索深挖
                                      ↓
                第 0a 步：拉过去 24h 已建话题（全状态）→ 去重对照
                                      ↓
                第 0a.5 步：过去 12h 审核中话题预过滤
                              （撤回 / 优化 title 后发布 / 保留观察）
                                      ↓
...
- **金额过小**：链上转账 < $100K / 融资 < $20M / 持仓变动 < $1M（无市场冲击）
- **空话标题**：触禁用词（"全解析" / "值得关注" / "速来" 等）或无具体标的
- **双轴硬不达标**：A=0 且 B=0（评估到的 12h 候选直接套 0b 双轴 mini 评分）
- **触发升温硬规则 >25% 偏差**：主体 narrative 已不成立（如"短拉 +40%"已回落到 +1.5%）—— desc 不可改 → 走撤回
