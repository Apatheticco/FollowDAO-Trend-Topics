...
> **🧩 系统碎片/重复检测（铁律 v2.7.2 — 2026-07-01 用户指令「mark 下、下次扫描也能扫到处理」）**：每轮扫 B0 时**主动跑两项健康检测，结果固定进收尾**（不被动等用户问）：
> - **① 同事件被拆成多碎片**：同一催化/同标的被系统拆成 ≥2 条不同 ID（如 2026-07-01 Circle 崩盘的 10240/10241/10246/10250）→ 收尾列「建议合并」：我方**新建/保留 1 条干净打包**，其余碎片按处置（自建重复可撤；**系统批碎片交 dev/团队后台清**）。
> - **② 同标题重复刷屏**：同一标题被系统批刷 ≥3 条（如 2026-06-30 MSTR「数字信贷框架变现BTC」×18）→ 收尾标「系统重复 N 条」。⚠️ **`update_trending_topic` 按标题模糊匹配，命中多条同名会直接报错、无 ID 定位入口 → 这类重复前端工具清不掉，只能列出交 dev 后台去重**（见下「console MCP quirk」）。
> - 检测**不因 >8h 豁免**（碎片/重复是"卫生问题"、非"追新脱锚"，8h 闸只管后者）；但清理动作仍守规矩：自建的按 8h 闸，系统批只列不擅动。
