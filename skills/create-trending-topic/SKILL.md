...
> **🔁 近 6h 上线话题退场复查（铁律 v2.6.8 — 2026-06-22 用户指令）**：每轮扫描**必从 B0 挑出 status=0 且发布 ≤6h 的话题逐条复查**（不只盯新候选，刚上线的也要维护）——① **脱锚/失实/时效过期 → 下架 `status=3`**（入表3）；② **数据旧但仍成立 → 改价/标题/keywords `update`**（入表2）；③ 仍准确 → 不动。复查口径同上表"0 上线 ≤24h"行，但**这是主动必跑步骤**，不是被动遇到才处理。系统自动批（10000001xxx）只扫明显脱锚错误，自建话题逐条过。
