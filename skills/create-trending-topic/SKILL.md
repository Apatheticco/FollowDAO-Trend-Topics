...
- **0️⃣ 素材去重前置（铁律 v2.6.2 — 2026-06-16 用户指令）**：用户提**单条素材**要建话题时，**create 前必先 `list_trending_topics` 扫 status=0(已上线)+status=2(审核中)**，**8h 内**有同标的/同事件的相关话题 → **优先 `update` 老话题**（更新数据点/标题/keywords），**不新建重复**；仅当 8h 内无相关话题才补新建。（这是 B/C 素材入口的固定第一步，与扫描模式的 0a 去重同源，但对单素材输入强制前置。）
...
> **🌏 非美个股价格盲区 → WebSearch 当场兜底（铁律 v2.6.4 — 2026-06-18 SK 海力士复盘）**：`followin.metrics` **对韩股/日股/欧股等非美个股无覆盖**（已实测 SK 海力士 `000660.KS`/`HXSCL`/`HYNIX`/`SKHYNIX` 全返空）。这类标的建题时**禁止拿新闻里的隔日盘中数直接进标题**（新闻往往是 T-1 的盘中快照，进标题即脱锚）——**必须当场 `WebSearch` 取当日实时价**（韩媒 dated 源口径互洽优先；英文聚合源若与韩媒不自洽则弃用）。性质同 quirk⑨（大宗裸 keyword 取不到换现货符号）、5/14 漏 H200（FMP 盲区 query 兜底）：**数据源有覆盖盲区时，盲区标的当场换源补全，不将就二手数。**
