...
> - **🆕 全池摊开 + >6h 冗余/反转破闸下架（v2.9.6 — 2026-07-15 用户指令「扫 list 时注意重复主题合并」，实测栽了两处）**：
>   - **① 去重审计摊全池、不止管理窗**：收尾去重检测**必扫全池 status=0+2 全量**（不只 `create_at ≤8h`）——2026-07-15 首扫我只摊了 ≤8h 窗、把 >8h 的半导体/光通信碎片簇省略成"交 dev"，被用户点名（银行例 10000002149 vs 10000002152）。全池摊开是收尾固定动作（见 §3 收尾清单第 6 块）。
>   - **② 同事件"单名 vs 板块全景"冗余 → 主动隐藏单名**（即使 >6h）：同 session 同标的被同时建了单名题+板块题（如 AMD 单名 10000002148 vs 半导体板块 10000002144、Citi 单名 10000002149 vs 银行板块 10000002152），**留板块全景+数字准的、隐藏单名冗余**。标题互异可唯一定位即可动手，不受 6h/8h 闸约束（团队本就在主动去重，实测系统已自隐 10149/10142）。
>   - **③ 盘中反转"错向老题" → 破 8h 闸下架**（属 v2.9.4 事实反转破闸的高频子类）：系统早盘按盘前/盘中快照建的"XX 板块下挫/上涨"题，若标的**收盘方向已反转**（2026-07-15 实测：SNDK-13%→收 +4.9%、COHR-5.3%→+1.1%、LITE-4.2%→+6.2%，CPI 走冷后存储/光通信整体反转），前台"集体下挫"即失实 → `status=3`。**判据 = fact-check 时拿收盘价对标题方向，反了就清**（不只校幅度、必校方向）。仅限标题互异可定位；同名多条仍交 dev。
...
- **建**：预览确认（不可跳过，确认前不调 MCP；**🤖 AUTO 模式除外**——跳预览但一律停 status=2，见 AUTO 协议）→ create（**不传 desc**；**keywords ≤4 个铁律 v2.7.4**——主标的 ticker+名 + 最核心 1-2 概念，砍次要实体/同义堆叠，别再塞 10+）→ **tag 校验（铁律 v2.9.5 — 系统自动打标器不可信，每轮必验）**：create 返回的 `matched_tags` **当场看**——① **≥7 位长 ID（尤其 11 位，如 34523806708/10478645335）= 系统自造的垃圾占位 tag**（正常 tag 是 5-6 位，如 CRCLX 522611），**必须 force-set 正确 tag 覆盖或清空**；② **错实体**（把标题里的机构/监管缩写当标的：Circle→`OCC`、SpaceX→`SPC`）同样覆盖；③ 无已知正确 tag（宏观事件/板块题/小币）→ **留空**，宁可空也不留错标（"tag 不硬塞"照旧）。**改法**：`update_trending_topic(tags=[{"id":正确ID}])`——传 `tags` 时以它为准、不会被 keywords 重匹配顶掉。**新实测到的 tag 补进 `data-quickref`**。→ **4.5 数据核实**（≥5 条走批量路径：1 次 metrics + 并行多源；**地缘/政策/突发自动加一轮 WebSearch 二次核**，见 v2.5.3）。
...
- **📊 报告表格化优先（铁律 v2.7.9 — 2026-07-01 用户指令「output 最好表格化」；2026-07-06 复申并加⑤）**：所有报告输出**默认用表格承载**，减少长散文/多层 bullet 堆叠——① 三件套（🆕候选 ID/标题/评分/缺口、🔧更新 ID/现状/改为、🚨撤回 ID/原因）用表；② 价格快照（标的/价/涨跌）用表；③ 已执行处置（ID/动作/结果）用表；④ 当前话题全列（ID/标题/status）用表；⑤ **待决策/待你拍板事项用表**（列 事项/内容/我的建议/待你决定；多条积压时**逐条一行**，别散在段落里让用户翻找——2026-07-06：用户连续几轮收尾后仍需专门说"列成表格"才看清待决项，说明散文式收尾不合格）。**一句话结论 + 表格**，能表格化的就别写成段落。仅纯叙述性判断（如某条为何降级）可少量散文。AUTO 模式尤其严格（无人复核，散文=没人看得清）。
