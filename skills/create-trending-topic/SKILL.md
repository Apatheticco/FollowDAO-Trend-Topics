...
- **建**：预览确认（不可跳过，确认前不调 MCP）→ create（**不传 desc**，落 status=2 审核）→ tag 校验（美股主动手动绑，查速查表）→ **4.5 数据核实**（≥5 条走批量路径：1 次 metrics + 并行多源；**地缘/政策/突发自动加一轮 WebSearch 二次核**，见 v2.5.3）→ **停在 status=2，不自动上线**。
- **发**：上线是**独立步骤**，**只有用户明确说"上线/发布/上"才 flip status=0**。建完 ≠ 发布——默认留审核，把待发清单列给用户等指令。（铁律 v2.5.2：禁止 create 后惯性自动 status=0）
- **收尾必列完整清单（铁律 v2.5.5）**：每轮扫描**结尾固定输出一份完整清单**，不口头带过、不省略。四块：
  1. **三件套全列**：🆕 候选（评分+缺口）｜🔧 待更新｜🚨 待撤/在审处置 —— 干轮如实写"无"，不灌水不硬报 0
  2. **本轮已执行**：建了哪些（ID+当前 status）/ 改了哪些 / 撤了哪些
  3. **⏸ 待用户决定**：待发清单（status=2 列表）+ 待批更新
...
[B 给素材] ──→ 第 1 步：字段提取 + 标题优化
                ...
                第 4.5 步：数据核实（强制）
                                      ↓
                第 5 步：停 status=2 审核（建完不自动上线）
                                      ↓
                第 6 步：⏸ 等用户明确"上线/发布" → flip status=0
```
