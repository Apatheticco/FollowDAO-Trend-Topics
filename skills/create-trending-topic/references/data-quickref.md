...
| ~~大师~~ | ~~2051856808348987697~~ | ❌ **已失效（v2.9.28 — 2026-08-05/06 两轮重探均失败）**：`list_tweets` 与 `list_timeline` 两种 action 都返回 `{"results":[],"total":0}`（非报错，是空数组），判定该 list 在 Followin MCP 侧已不存在或无数据。**首扫不再跑第 3 栈**（改二栈，见 SKILL §1）；拿到新的大师 list_id 再补回本表并恢复三栈。|
