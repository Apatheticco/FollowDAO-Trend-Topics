...
### 🧹 灌水形态特征闸（v2.9.77 —— 数据驱动前置，先扫形态再逐条细审）

> **依据**：第 0.5 层全量回填基线（1215 行）——撤题 137 条可分类中 `A闸不过-灌水` **56 条占 41%**、是第二名的 4 倍，且形态高度同质。**每轮人工清同一类 ＝ 预过滤没拦住**；本闸把这 41% 的处置从"逐条想"变成"先扫形态"。
>
> 在审池逐条过以下形态表，**命中即进撤候选**（仍走一遍 meme 豁免与三类正当撤因核对——形态闸是快筛不是终审）：
>
> | 形态 | 特征（标题层面可判） | 历史实例 |
> |------|--------------------|---------|
> | KOL 观点 | 人名/机构名开头＋冒号/「预测/称/表示/看好」，无当日价格数字 | 何一、Ansem、CZ、CryptoQuant、Tom Lee |
> | 产品/路线图公告 | 「推出/发布/上线（产品名）/打造/推迟/纳入路线图」，无可交易标的定价 | Coinbase CEEcil、AiFi 愿景、POD 推迟 |
> | vanity 指标 | 「TVL/持仓/用户数/账户数/储备金/mNAV 突破 X」，指标不对价格 | TradeXYZ 40 万用户、PURR mNAV、TRON 4 亿账户 |
> | 融资无标的 | 「完成 X 轮融资」且主体无可交易代币/股票 | fomo 联创、Hivemind、Socure |
> | 悬念疑问 | 问号收尾＋「能否/会否/还能/引爆/热议」（→ 疑问句闸 v2.9.68 判去问号后剩什么） | STONK「热度能否延续？」 |
> | 立法/程序进展 | 无生效日期与定价标的的立法/诉讼/听证进程 | 波兰法案重表决、Tornado 审判推迟 |
>
> **不命中形态表的才值得逐条细审**——把细审时间花在真正模糊的题上。**豁免边界**：命中形态但**同一标题里带实测价格/幅度数字**的，不走本闸（那是行情题，进正常评分）。
