...
> 最常被漏：**双栈 list_timeline**（主 2046… / 科技AI 2051854…）。下方「判定细节附录」只补批次表装不下的判定细节。
...
> 🔻 **裸 feed TG 已砍（v2.6.6）**：2026-06-20 工作日+周末两批实测，空 query / category browse 的 TG 返回**几乎全是 bot 噪音**——爆仓 bot（微额 $50K）、匿名巨鲸流水（撞链上红线）、喊单/meme shill bot，topic-worthy≈0；少数实质内容（FOMC/STRC/监管）media 与双栈 list 已覆盖。**TG 唯一残余独有价值 = 安全/exploit 早警**（Axelar $4.7M、Aztec $2M 偶尔比 media 快几分钟）。故 5-cat 并行 browse 全删，只留**一条 exploit 探针**；叙事/上币/监管/鲸鱼全交给双栈 list + W2 media query（本就覆盖）。
...
### 🚨 强制铁律：Twitter 双栈 list 不可跳过（v2.2.4 实战教训；TG 已降级 v2.6.6）

实战教训（5/19）：漏掉 Twitter list 直接缺 5+ 强候选——megaevent 常只在 list 出现，新闻通道捞不到，故 §1 把**双栈 list 列为必跑**（详见 LESSONS.md）。**TG 已于 v2.6.6 降级**：2026-06-20 实测裸 feed TG 全 bot 噪音（出真候选的是 list，不是 TG），仅保留 1 条 exploit 探针，**不再算"不可跳过"**——双栈 list 才是必跑核心。

**失败处置**：任一批次失败 → **重试 1 次**；主进程仍失败（尤其 followin `session initialization`）→ **走 news 降级梯：Agent 子进程代跑**（子进程独立 session 绕过主 server 抖动，v2.8.4）；子进程也失败才标"⚠️ 通道缺失"。**任一必跑通道(news/TG/双栈)缺且补不齐 → 触发半盲熔断（SKILL §0 v2.8.4）：报告标「半盲扫·结论存疑」，禁报干净干轮**。不能直接进 0b。

**覆盖自检铁律（v2.8.2 — 取代旧"候选数量铁律"）**：跑完执行卡 §1 全部批次后，进 0b 前只做**覆盖自检**：候选稀少 + 当日明显有大事 → 对照 §1 批次表逐波查是否漏跑（最常漏：双栈 list）；通道无漏 → 如实报干轮。**不设数字目标、不为凑数扩窗**（扩窗仅当确认某通道漏跑/失败需补时使用）。
...
N：首扫 主15/科技12；刷新 主10/科技8。双栈 list 各跑一次。主进程拿到 dump 后按双轨制（velocity top 7 + recency top 3 + 30min 绝对保护）粗排出最终候选。
